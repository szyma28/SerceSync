part of 'manager_dashboard.dart';
            ...
            children: [
              ...
              if (rosterPanels.isNotEmpty) ...[
                ...
                if (!isTwoColumn)
                  ...
                  Wrap(spacing: 12, runSpacing: 12, children: rosterPanels),
              ],
            ],
          );
        },
      ...
      child: Column(
        ...
        children: [
          ...
          AnimatedSwitcher(
            ...
            child: items.isEmpty
                ? Container(
                    key: const ValueKey('exception-feed-empty'),
                    width: double.infinity,
                    padding: const EdgeInsets.all(18),
                    decoration: BoxDecoration(
                ...
                : AnimatedSize(
                    key: ValueKey('exception-feed-${items.length}'),
                    duration: const Duration(milliseconds: 180),
                      ...
                      children: [
                        for (final item in items) ...[
                          _ExceptionFeedRow(
                            ...
                            item: item,
                            isActionPending: pendingIncidentIds.contains(
                              item.id,
                            ),
                ...
                if (item.isIncident &&
                  ...
                  AnimatedSwitcher(
                    ...
                    child: isActionPending
                        ? const SizedBox(
                            key: ValueKey('incident-action-loading'),
                            width: 20,
                            height: 20,
                            child: CircularProgressIndicator(strokeWidth: 2),
                          )
                        : Wrap(
                            key: ValueKey('incident-actions-${item.id}'),
                            spacing: 10,
                            runSpacing: 10,
                            children: [
                              if (item.canAcknowledge)
                                OutlinedButton(
                                  onPressed: () => onAcknowledgeIncident(
                                    item.id,
                                    item.shiftId,
                                  ),
      ...
      children: const [
        ManagerSkeletonCard(
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              ManagerSkeletonBlock(height: 18, width: 160),
              SizedBox(height: 14),
