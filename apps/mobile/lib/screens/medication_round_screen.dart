...
class _MedicationRoundScreenState extends State<MedicationRoundScreen> {
    ...
    try {
      switch (action) {
        case _MedicationRoundAction.administer:
          await widget.apiClient.administerMedicationDose(
            accessToken: widget.accessToken,
        ...
        case _MedicationRoundAction.refuse:
          await widget.apiClient.refuseMedicationDose(
            accessToken: widget.accessToken,
            doseInstanceId: item.id,
            reason: draft.reason!,
            notes: draft.notes,
        ...
        case _MedicationRoundAction.omit:
          await widget.apiClient.omitMedicationDose(
            accessToken: widget.accessToken,
            doseInstanceId: item.id,
            reason: draft.reason!,
            notes: draft.notes,
        ...
        case _MedicationRoundAction.delay:
          await widget.apiClient.delayMedicationDose(
            accessToken: widget.accessToken,
            doseInstanceId: item.id,
            reason: draft.reason!,
            notes: draft.notes,
        ...
        case _MedicationRoundAction.notAvailable:
          await widget.apiClient.markMedicationNotAvailable(
            accessToken: widget.accessToken,
            doseInstanceId: item.id,
            reason: draft.reason!,
            notes: draft.notes,
        ...
        case _MedicationRoundAction.hold:
          await widget.apiClient.holdMedicationDose(
            accessToken: widget.accessToken,
            doseInstanceId: item.id,
            reason: draft.reason!,
            notes: draft.notes,
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('${action.label} recorded for ${item.medicationName}.'),
        ),
      );
    } on ApiException catch (error) {
      if (!mounted) return;
  ...
  Future<void> _openAddNoteDialog(MedicationRoundItem item) async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Medication note saved for ${item.residentName}.'),
        ),
      );
    } on ApiException catch (error) {
      if (!mounted) return;
